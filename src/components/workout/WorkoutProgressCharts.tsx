  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      for (const ex of exercises) {
        ...
        grouped[ex.exercise_name].push({ date: format(new Date(date), "MMM d"), maxWeight, maxReps });
      }
        ...
        <>
          <div className="flex flex-wrap gap-2">
            {exerciseNames.map((name) => (
              <Button key={name} variant={selectedExercise === name ? "default" : "outline"} size="sm" onClick={() => setSelectedExercise(name)}>{name}</Button>
            ))}
          </div>
          {selectedExercise && chartData && (
            <div className="space-y-4">
              <Card>
                <CardHeader><CardTitle className="font-display text-base">{t.workouts.weightProgression}</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-56">
              ...
              <Card>
                <CardHeader><CardTitle className="font-display text-base">{t.workouts.repsProgression}</CardTitle></CardHeader>
                <CardContent>
                  <div className="h-56">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={chartData}>
