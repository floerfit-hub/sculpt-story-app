      ...
      {exercises.map((ex, exIdx) => (
        <Card key={exIdx}>
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div><p className="font-display font-semibold">{t.exerciseNames[ex.name] || ex.name}</p><p className="text-xs text-muted-foreground">{t.muscleGroups[ex.muscleGroup as keyof typeof t.muscleGroups] || ex.muscleGroup}</p></div>
