  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const mapped = wData.map((w) => ({
        ...w,
        exercises: ((exData ?? []).filter((e) => e.workout_id === w.id) as unknown as WorkoutExerciseRow[]),
      }));
      setWorkouts(mapped);
      setLoading(false);
    };
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-display font-bold">Workout History</h2>
      </div>
      ...
      {!loading && workouts.length === 0 && (
        <p className="py-12 text-center text-muted-foreground">No workouts logged yet.</p>
      )}
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-display font-semibold">
                    {format(new Date(w.started_at), "EEEE, MMM d yyyy")}
                  </p>
