...
const AppLayout = ({ children }: { children: ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="px-5 py-6 pb-24 lg:pb-8 max-w-2xl mx-auto" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>{children}</main>

      <nav className="fixed bottom-0 left-0 right-0 z-50 glass-strong">
        <div className="flex justify-around pb-[env(safe-area-inset-bottom)] pt-2 pb-3 max-w-lg mx-auto">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex flex-col items-center gap-1 px-3 py-1.5 text-[11px] transition-all duration-200 min-w-[56px] rounded-xl ${
                  active
